    ...
    <div class="page-content q-gutter-lg">
      ...
      <div class="row items-center">
        <div class="col">
          ...
          <div class="text-body2 text-grey-7">Confirme quem participa dos próximos encontros.</div>
        </div>
        <q-btn v-if="canManagePlayers" color="primary" icon="event" label="Novo jogo" @click="gameDialog = true" />
      </div>
      ...
      <q-banner v-if="!currentUser" rounded class="bg-blue-1 text-primary">
        Entre na sua conta para responder à chamada. A agenda e a lista de presença continuam públicas.
      ...
      <q-banner v-if="!loading && !games.length" rounded class="bg-grey-2 text-grey-8">Nenhum jogo agendado.</q-banner>
      ...
      <div class="games-grid">
        <q-card v-for="game in games" :key="game.id" flat bordered>
          <q-card-section>
            ...
            <div class="text-subtitle2"><q-icon name="schedule" /> {{ formatDate(game.startsAt) }}</div>
            <div v-if="game.location" class="text-body2"><q-icon name="place" /> {{ game.location }}</div>
            <div v-if="game.notes" class="text-body2 q-mt-sm">{{ game.notes }}</div>
          </q-card-section>
          ...
          <q-card-section>
            ...
            <div class="row q-gutter-sm">
              <q-btn
                ...
                :disable="!currentUser || !selectedSubject"
          ...
          <q-separator />
          <q-card-section>
            <div class="text-subtitle2">Confirmados ({{ grouped(game.id).going.length }})</div>
            <div class="q-gutter-xs q-mt-xs">
              <q-chip v-for="attendance in grouped(game.id).going" :key="attendance.userId" color="green-1" text-color="positive">
                {{ attendance.displayName }}
              </q-chip>
              <span v-if="!grouped(game.id).going.length" class="text-grey-6">Ninguém confirmou ainda.</span>
            </div>
            <div v-if="grouped(game.id).maybe.length" class="text-caption text-grey-7 q-mt-sm">
              Talvez: {{ grouped(game.id).maybe.map((item) => item.displayName).join(', ') }}
            </div>
          </q-card-section>
        </q-card>
      </div>
      ...
      <q-card style="width: 460px; max-width: 95vw">
        ...
        <q-form @submit.prevent="saveGame">
          <q-card-section class="q-gutter-md">
            <q-input v-model.trim="gameDraft.title" label="Título" :rules="[(value) => Boolean(value) || 'Informe o título']" />
            <q-input v-model="gameDraft.startsAt" type="datetime-local" label="Data e hora" stack-label />
            <q-input v-model.trim="gameDraft.location" label="Local" />
            <q-input v-model.trim="gameDraft.notes" type="textarea" label="Observações" />
          </q-card-section>
          <q-card-actions align="right">
            <q-btn flat label="Cancelar" v-close-popup />
            <q-btn color="primary" label="Agendar" type="submit" :loading="saving" />
          </q-card-actions>
        </q-form>
      </q-card>
    </q-dialog>
...
const gameDraft = reactive({ title: 'Vôlei', location: '', notes: '', startsAt: now.toISOString().slice(0, 16) });

/* ===========================================================
   PESSOA REPRESENTADA NA RESPOSTA
=========================================================== */

const selectedSubject = computed(() => subjects.value.find((subject) => subject.uid === selectedSubjectId.value));
const subjectOptions = computed(() => subjects.value.map((subject) => ({
  label: `${subject.displayName}${subject.email ? ` (${subject.email})` : ''}`,
  value: subject.uid,
})));
...
onMounted(() => {
  unsubscribeGames = subscribeToGames((freshGames) => {
    ...
    freshGames.forEach((game) => {
      if (!attendanceSubscriptions.has(game.id)) {
        attendanceSubscriptions.set(game.id, subscribeToAttendances(game.id, (items) => { attendances[game.id] = items; }));
      }
    });
  }, () => {
    loading.value = false;
...
watch([currentUser, currentRole], async ([user]) => {
  if (!user) {
    subjects.value = [];
    selectedSubjectId.value = '';
    return;
  }
  const self = { uid: user.uid, displayName: user.displayName ?? user.email ?? 'Você', email: user.email ?? '' };
  try {
    const others = canManagePlayers.value ? await listUsers() : await listDelegators(user.uid);
    subjects.value = [self, ...others.filter((subject) => subject.uid !== user.uid)];
  } catch {
    subjects.value = [self];
  }
  if (!subjects.value.some((subject) => subject.uid === selectedSubjectId.value)) selectedSubjectId.value = user.uid;
}, { immediate: true });
...
function subjectStatus(gameId: string) {
  return (attendances[gameId] ?? []).find((item) => item.userId === selectedSubjectId.value)?.status;
}

async function respond(gameId: string, status: AttendanceStatus) {
  if (!currentUser.value || !selectedSubject.value) return;
  try {
    await setAttendance(gameId, selectedSubject.value, status, currentUser.value);
    $q.notify({ type: 'positive', message: 'Presença atualizada.' });
  } catch {
    $q.notify({ type: 'negative', message: 'Você não tem permissão para responder por essa pessoa.' });
  }
}
...
async function saveGame() {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'Não foi possível agendar o jogo.' });
  } finally {
    saving.value = false;
  }
}
...
.games-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 18px; }
</style>
